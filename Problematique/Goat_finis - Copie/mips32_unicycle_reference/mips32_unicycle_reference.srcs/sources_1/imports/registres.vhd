...
architecture comport of BancRegistres is
    signal regs: RAM(0 to 31) := (29 => X"100103FC", -- registre $SP
                                others => (others => '0'));
    signal regs_vector: RAM_Vector(0 to 3) := (others => (others => '0'));
begin
    process( clk )
    begin
        if clk='1' and clk'event then
            ...
            if i_WE_Vector = '1' and reset = '0' and i_Wdest /= "00000" then
                regs_vector( to_integer( unsigned(i_WDest))) <= i_Wr_DAT;
            end if;
        end if;
    end process;

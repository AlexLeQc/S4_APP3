...
architecture Behavioral of mips_datapath_pipeline is
    ...
    signal r_IF_ID_Instruction    : std_logic_vector(31 downto 0);
    signal r_IF_ID_opcode         : std_logic_vector( 5 downto 0);
    signal r_IF_ID_rs             : std_logic_vector( 4 downto 0);
    signal r_IF_ID_rt             : std_logic_vector( 4 downto 0);
    signal r_IF_ID_rd             : std_logic_vector( 4 downto 0);
    signal r_IF_ID_shamt          : std_logic_vector( 4 downto 0);
    signal r_IF_ID_instr_funct    : std_logic_vector( 5 downto 0);
    signal r_IF_ID_imm16          : std_logic_vector(15 downto 0);
    signal r_IF_ID_jump_field     : std_logic_vector(25 downto 0);
    ...
    signal s_IF_ID_imm_extended          : std_logic_vector(31 downto 0);
    signal s_EX_MEM_imm_extended_shifted  : std_logic_vector(31 downto 0);
	
    signal s_Reg_Wr_Data           : std_logic_vector(31 downto 0);
    signal s_MemoryReadData        : std_logic_vector(31 downto 0);
    signal s_AluA_data             : std_logic_vector(31 downto 0);
	...
	-- Contrôles
	-- ID stage
	signal r_IF_ID_adresse_PC_plus_4     : std_logic_vector(31 downto 0);
	-- EX stage endpoint
	signal r_ID_EX_alu_funct   	: std_logic_vector(3 downto 0);
	signal r_ID_EX_ALUSrc      	: std_logic;
	signal r_ID_EX_RegDst      	: std_logic;
	-- MEM stage endpoint
	signal r_ID_EX_branch      	: std_logic;
	signal r_ID_EX_MemRead		  	: std_logic;
	signal r_ID_EX_MemWrite	  	: std_logic;
	signal r_ID_EX_jump   	  		: std_logic;
	signal r_ID_EX_jump_register 	: std_logic;
    signal r_ID_EX_jump_field      : std_logic_vector(25 downto 0);
	-- WB stage endpoint
	signal r_ID_EX_jump_link   	: std_logic;
	signal r_ID_EX_RegWrite    	: std_logic;
	signal r_ID_EX_MemtoReg    	: std_logic;
    signal r_ID_EX_adresse_PC_plus_4     : std_logic_vector(31 downto 0);
	
	-- Signaux générés
    signal r_ID_EX_imm_extended    : std_logic_vector(31 downto 0);
	signal r_ID_EX_reg_data1		: std_logic_vector(31 downto 0);
	signal r_ID_EX_reg_data2		: std_logic_vector(31 downto 0);
	signal r_ID_EX_rs				: std_logic_vector(4 downto 0);
	signal r_ID_EX_rt				: std_logic_vector(4 downto 0);
	signal r_ID_EX_rd				: std_logic_vector(4 downto 0);
	signal r_ID_EX_shamt			: std_logic_vector(4 downto 0);
	
	-- Registres étage MEM
	-- MEM stage endpoint
	signal r_EX_MEM_branch      	: std_logic;
	signal r_EX_MEM_MemRead	  	: std_logic;
	signal r_EX_MEM_MemWrite	  	: std_logic;
	signal r_EX_MEM_jump   	  	: std_logic;
	signal r_EX_MEM_jump_register 	: std_logic;
    signal r_EX_MEM_jump_field     : std_logic_vector(25 downto 0);
	-- WB stage endpoint
	signal r_EX_MEM_jump_link   	: std_logic;
	signal r_EX_MEM_RegWrite    	: std_logic;
	signal r_EX_MEM_MemtoReg    	: std_logic;
    signal r_EX_MEM_adresse_PC_plus_4     : std_logic_vector(31 downto 0);
	
	
	-- Signaux générés
    signal r_EX_MEM_imm_extended          : std_logic_vector(31 downto 0);
    signal r_EX_MEM_imm_extended_shifted  : std_logic_vector(31 downto 0);
	signal r_EX_MEM_AluResult				   : std_logic_vector(31 downto 0);
	signal r_EX_MEM_zero					: std_logic;
	signal r_EX_MEM_reg_data1			      : std_logic_vector(31 downto 0);
	signal r_EX_MEM_reg_data2			      : std_logic_vector(31 downto 0);
	signal r_EX_MEM_RegisterRd	: std_logic_vector(4 downto 0);
	
	-- Registres étage MEM
	-- WB stage endpoint
	signal r_MEM_WB_jump_link	 	: std_logic;
	signal r_MEM_WB_RegWrite    	: std_logic;
	signal r_MEM_WB_MemtoReg    	: std_logic;
    signal r_MEM_WB_adresse_PC_plus_4     : std_logic_vector(31 downto 0);
	signal r_MEM_WB_AluResult			      : std_logic_vector(31 downto 0);
	signal r_MEM_WB_MemoryReadData			      : std_logic_vector(31 downto 0);
	signal r_MEM_WB_RegisterRd	: std_logic_vector(4 downto 0);
...
s_adresse_branche				<= std_logic_vector(unsigned(s_EX_MEM_imm_extended_shifted) + unsigned(r_EX_MEM_adresse_PC_plus_4));
...
------------------------------------------------------------------------
-- extension de signe
s_IF_ID_imm_extended <= std_logic_vector(resize(  signed(r_IF_ID_imm16),32)) when i_SignExtend = '1' else -- extension de signe à 32 bits
				  std_logic_vector(resize(unsigned(r_IF_ID_imm16),32)); 


s_AluA_data <= r_ID_EX_reg_data1; -- mux prévu pour l'unité d'avancement 

-- Mux pour immédiats
s_AluB_data <= r_ID_EX_imm_extended when r_ID_EX_ALUSrc = '1' else r_ID_EX_reg_data2; -- modifier pour ajouter l'unité d'avancement. Attention aux priorités pour l'immédiat.

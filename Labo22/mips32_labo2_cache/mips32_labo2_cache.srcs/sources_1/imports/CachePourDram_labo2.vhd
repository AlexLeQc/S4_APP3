...
Port ( 
	...
	i_dram_WordIndexSel : in std_logic_vector(2 downto 0);
...
architecture Behavioral of CachePourDram is
	...
	constant c_Tag_BitCount	   : integer := 25; --???
	constant c_BlockIndex_BitCount   : integer := 4;--???
	constant c_BlockOffset_BitCount   : integer := 1;--???
	constant c_ByteOffset_BitCount   : integer := 2;--???
    ...
    type t_cache_data is array (natural range <>, natural range <>) of std_logic_vector(31 downto 0);
	type t_ram_tag    is array (natural range <>) of std_logic_vector (c_Tag_BitCount - 1 downto 0);
	
	-- mémoire interne de la cache (bit de validité, étiquette et mémoire de données
	signal cache_validity_array			: std_logic_vector(c_BlockCount - 1 downto 0) := (others => '0');
	...
	signal s_addr_tag_bits			: std_logic_vector(c_Tag_BitCount - 1 downto 0);
	signal s_addr_BlockIndex_bits		: std_logic_vector(c_BlockIndex_BitCount - 1 downto 0);
	signal s_addr_BlockOffset_bits	: std_logic_vector(c_BlockOffset_BitCount - 1 downto 0);
	signal s_addr_ByteOffset_bits	: std_logic_vector(c_ByteOffset_BitCount - 1 downto 0);
	---------------------------------------------------------------------------------------------------------
	
	signal s_CacheMiss			: std_logic;
	signal s_RequestedBlockValid     : std_logic;
	signal s_RequestedBlockTagMatch  : std_logic;
	
	
	signal s_LocalCacheWrite			: std_logic;
	signal s_buffer_write_enable   : std_logic;
	signal s_buffer_empty          : std_logic;
	signal s_buffer_full           : std_logic; -- max 16 mots    
...
s_addr_tag_bits			<= i_cpu_Addresse(31 downto 7);
s_addr_BlockIndex_bits	<= i_cpu_Addresse(6 downto 3);
s_addr_BlockOffset_bits	<= i_cpu_Addresse(2 downto 2);
s_addr_ByteOffset_bits	<= i_cpu_Addresse(1 downto 0);
...
-- note: (0 downto 0) est un std_logic_vector de 1 bit
----------------------------------------------------------------------
